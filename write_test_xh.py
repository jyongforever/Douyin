...
def video_write(author_name):
    if not os.path.exists("Video/喜欢/%s" % author_name):
        os.makedirs("Video/喜欢/%s" % author_name)
    # 记录下载过的视频url
    # v = open('video.txt','a+')
    # v.seek(0)
    # v_list = v.read().split()
    ...
    for url in video_list:
        ...
        # v.write(vname + '\n')
        with open('Video/喜欢/%s/%s.mp4' % (author_name,vname),'wb') as f:
            ...
            if content_size == 169 or content_size == 0:
                os.remove('Video/喜欢/%s.mp4' % vname)
                print("无法播放视频已删除")
                continue
            f.write(res.content)
        n += 1
